...
| error             | 自定义图像加载失败的内容                             | -                                                                                                                                                                                                                                |
| viewer            | 当图像预览时自定义内容                              | -                                                                                                                                                                                                                                |
| progress ^(2.9.4) | 图片预览时自定义进度条内容 （优先级高于 `show-progress` 属性） | ^[object]`{ activeIndex: number, total: number }`                                                                                                                                                                                |
| toolbar ^(2.9.4)  | 当图像预览时，自定义工具栏内容                          | ^[object]`{actions: (action: ImageViewerAction, options?: ImageViewerActionOptions ) => void, prev: ()=> void, next: () => void,reset: () => void, activeIndex: number }, setActiveItem: (index: number) => void` |
